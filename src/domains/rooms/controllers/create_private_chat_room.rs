...
pub async fn create_room(
        ...
        .await
        {
            ...
            Ok(None) => {
                ...
                return (
                    ...
                    Json(Response {
                        response_message: "Room creation error - failed to fetch room co_member".to_string(),
    ...
    match sqlx::query_as::<_, Room>(
        "SELECT id, room_name, is_group, created_by
                FROM rooms
                WHERE created_by = $1
                    AND room_name = $2
                    AND is_group =  $3
    ...
    let room = match sqlx::query_as::<_, Room>(
        r#"
        INSERT INTO rooms (room_name, is_group, created_by)
        VALUES ($1, $2, $3)
        RETURNING id, room_name, is_group, created_by
        "#,
    )
    .bind(&co_member.full_name)
    .bind(payload.is_group)
    .bind(created_by)
    .fetch_one(&state.db)
    .await
    {
        ...
        Err(e) => {
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(Response {
                    response_message: "Failed to create room".into(),
                    response: None,
    ...
    .await
    {
        ...
        Err(e) => {
            ...
            return (
                StatusCode::INTERNAL_SERVER_ERROR,
                Json(Response {
                    response_message: "Failed to create room member 1".into(),
                    response: None,
                    error: Some(format!("Room member creation error : {}", e)),
                }),
            );
